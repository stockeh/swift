"""
python -m swift.data.process_one_step_era5 \
    --root_dir /eagle/MDClimSim/tungnd/data/wb2/6h_32_64 \
    --save_dir /eagle/projects/MDClimSim/jstock/data/wb2/5.625deg_1_step_6hr_h5df \
    --start_year 2020 \
    --end_year 2021 \
    --split test
...
parser.add_argument(
    "--save_dir", type=str, required=True, help="Directory to save regridded files."
)
parser.add_argument(
    "--start_year", type=int, default=1979, help="Start year for the data range."
)
parser.add_argument(
    "--end_year", type=int, default=2019, help="End year for the data range."
)
parser.add_argument(
    "--split",
    type=str,
...
parser.add_argument(
    ...
    type=int,
    default=10,
    help="Chunk size for reading datasets (default=10).",
)
parser.add_argument(
    "--workers",
    type=int,
...
def process_year(
    ...
    ds_sample = xr.open_dataset(
        os.path.join(root_dir, list_single_vars[0], f"{year}.nc")
    )
    ...
    for chunk_id in tqdm(range(n_chunks), desc="chunks", position=1, leave=False):
        ...
        for var in list_single_vars + list_pressure_vars:
            ds = ds_dict[var].isel(
                time=slice(chunk_id * chunk_size, (chunk_id + 1) * chunk_size)
            )
        ...
        for i in tqdm(
            range(len(list_time_stamps)),
            desc="time stamps",
            position=2,
            leave=False,
        ):
            data_dict = {"input": {"time": str(list_time_stamps[i])}}
            ...
            with h5py.File(
                os.path.join(save_dir_split, f"{year}_{idx_in_year:04}.h5"),
                "w",
                libver="latest",
            ) as f:
...
def create_one_step_dataset(
    root_dir, save_dir, split, years, list_vars, chunk_size=None, workers=None
):
    ...
    list_single_vars = [
        v for v in list_vars if v in SINGLE_LEVEL_VARS and v not in CONSTANTS
    ]
    ...
    # parallel map over years
    fn = partial(
        process_year,
        root_dir=root_dir,
        save_dir_split=save_dir_split,
        list_constant_vars=list_constant_vars,
        list_single_vars=list_single_vars,
        list_pressure_vars=list_pressure_vars,
        chunk_size=chunk_size,
    )
    n_workers = min(workers, len(years))
    with ProcessPoolExecutor(max_workers=n_workers) as exe:
        list(tqdm(exe.map(fn, years), total=len(years), desc="years", position=0))
